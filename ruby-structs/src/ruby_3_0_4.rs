...
impl ::std::fmt::Debug for rb_method_definition_struct {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write ! (f , "rb_method_definition_struct {{ type : {:?}, alias_count : {:?}, complemented_count : {:?}, no_redef_warning : {:?}, body: {:?}, original_id: {:?}, method_serial: {:?} }}" , self . type_ () , self . alias_count () , self . complemented_count () , self . no_redef_warning () , self . body , self . original_id , self . method_serial)
    }
}
