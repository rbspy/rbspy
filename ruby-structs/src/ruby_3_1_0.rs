...
impl ::std::fmt::Debug for rb_method_definition_struct {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write ! (f , "rb_method_definition_struct {{ type : {:?}, iseq_overload : {:?}, alias_count : {:?}, complemented_count : {:?}, body: {:?}, original_id: {:?}, method_serial: {:?} }}" , self . type_ () , self . iseq_overload () , self . alias_count () , self . complemented_count () , self . body , self . original_id , self . method_serial)
    }
}
