...
pub struct rb_vm_struct__bindgen_ty_1 {
    ...
    pub safe: [::std::os::raw::c_uchar; 32usize],
}
#[test]
...
impl ::std::fmt::Debug for rb_vm_tag {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write!(
            f,
            "rb_vm_tag {{ tag: {:?}, retval: {:?}, buf: [{}], prev: {:?}, state: {:?} }}",
            self.tag,
            self.retval,
            self.buf
                .iter()
...
impl ::std::fmt::Debug for rb_execution_context_struct {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        write ! (f , "rb_execution_context_struct {{ vm_stack: {:?}, vm_stack_size: {:?}, cfp: {:?}, tag: {:?}, protect_tag: {:?}, safe_level: {:?}, raised_flag: {:?}, interrupt_flag: {:?}, interrupt_mask: {:?}, fiber_ptr: {:?}, thread_ptr: {:?}, local_storage: {:?}, local_storage_recursive_hash: {:?}, local_storage_recursive_hash_for_trace: {:?}, root_lep: {:?}, root_svar: {:?}, ensure_list: {:?}, trace_arg: {:?}, errinfo: {:?}, passed_block_handler: {:?}, passed_bmethod_me: {:?}, method_missing_reason: {:?}, machine: {:?} }}" , self . vm_stack , self . vm_stack_size , self . cfp , self . tag , self . protect_tag , self . safe_level , self . raised_flag , self . interrupt_flag , self . interrupt_mask , self . fiber_ptr , self . thread_ptr , self . local_storage , self . local_storage_recursive_hash , self . local_storage_recursive_hash_for_trace , self . root_lep , self . root_svar , self . ensure_list , self . trace_arg , self . errinfo , self . passed_block_handler , self . passed_bmethod_me , self . method_missing_reason , self . machine)
    }
}
