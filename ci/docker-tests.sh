...
do
   ...
   echo -n "${distro}... "
   docker run -v=/tmp/artifacts:/stuff rb-stacktrace-$distro /stuff/rbspy record /usr/bin/ruby /stuff/short_program.rb
done
...
do
   echo -n "${version}... "
   docker run -v=/tmp/artifacts:/stuff -t jvns/rbspy-ci:ubuntu1604 /stuff/rbspy record --file /dev/null  /root/.rbenv/versions/$version/bin/ruby /stuff/short_program.rb
done
