...
do
   echo -n "${version}... "
   docker run -v=/tmp/artifacts:/stuff -t jvns/rbspy-ci:ubuntu1604 env RUST_BACKTRACE=1 /stuff/rbspy record --file /dev/null  /root/.rbenv/versions/$version/bin/ruby /stuff/short_program.rb
done
